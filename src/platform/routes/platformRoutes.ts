...
const parseRecord = (value: unknown, fieldName: string): Record<string, unknown> | undefined => {
  ...
  if (typeof value !== 'object' || Array.isArray(value)) {
    const error = new Error(`${fieldName} must be an object.`) as Error & { status: number; code: string };
    error.status = 400;
...
export const createPlatformRouter = (service = new PlatformService()): Router => {
  ...
  router.get('/workloads', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const workloads = await service.listWorkloads(ownerId);
    res.json({ workloads });
  }));

  router.post('/workloads', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.get('/workloads/:workloadId', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const workload = await service.getWorkload(req.params.workloadId, ownerId);
    res.json({ workload });
  }));

  router.post('/workloads/:workloadId/runtimes', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.get('/runtimes', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const runtimes = await service.listRuntimes(ownerId);
    res.json({ runtimes });
  }));

  router.get('/runtimes/:runtimeId', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const runtime = await service.getRuntime(req.params.runtimeId, ownerId);
    res.json({ runtime });
  }));

  router.get('/runtimes/:runtimeId/history', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const history = await service.listRuntimeHistory(req.params.runtimeId, ownerId);
    res.json({ history });
  }));

  router.get('/runtimes/:runtimeId/logs', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.query.ownerId === 'string' ? req.query.ownerId : undefined;
    const logs = await service.listRuntimeLogs(req.params.runtimeId, ownerId);
    res.json({ logs });
  }));

  router.post('/runtimes/:runtimeId/stop', asyncHandler(async (req: Request, res: Response) => {
  ...
  router.delete('/runtimes/:runtimeId', asyncHandler(async (req: Request, res: Response) => {
    const ownerId = typeof req.body.ownerId === 'string' ? req.body.ownerId : undefined;
    const runtime = await service.deleteRuntime(req.params.runtimeId, req.body.requestedBy, ownerId);
    res.json({ runtime });
  }));
